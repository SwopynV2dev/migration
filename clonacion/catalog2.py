...
try:
    with source_conn.cursor(dictionary=True) as source_cursor:
        query = "SELECT * FROM plague_categories WHERE profile_job_center_id = %s"
        source_cursor.execute(query, (id_company_job_center,))
        rows = source_cursor.fetchall()
        for row in rows:
            data = {
            ...
            response = requests.post(api_url+"plague-categories/", headers=headers, json=data)
         
except mysql.connector.Error as e:
    print(f"Error con la tabla plague_categories: {e}")
except Exception as ex:
    print(f"Error inesperado con la tabla plague_categories: {ex}")



try:
    with source_conn.cursor(dictionary=True) as source_cursor:
        query = "SELECT * FROM type_stations WHERE profile_job_center_id = %s"
        source_cursor.execute(query, (id_company_job_center,))
        rows = source_cursor.fetchall()
        for row in rows:
            data = {
                    "name": row.get("name"),
                    "activity_field": True,
                    "conditions_field": True,
                    "observations_field": True,
                    "corrective_actions_field": True,
                    "species_count_field": False,
                    "is_catch_field": False,
                     "job_center" : uuid_nuevo
                    }
            response = requests.post(api_url+"station-types/", headers=headers, json=data)

except mysql.connector.Error as e:
    print(f"Error con la tabla type_stations: {e}")
except Exception as ex:
    print(f"Error inesperado con la tabla type_stations: {ex}")
...
SELECT  
    pt.id, 
    pt.name AS plague_name, 
    pt.profile_job_center_id, 
    pt.plague_key,
    pt.id_categorie, 
    ...
    pt.updated_at
FROM 
    pwa_prod.plague_types pt
LEFT JOIN 
    pwa_prod.plague_categories c
ON 
    pt.id_categorie = c.id
WHERE
    pt.profile_job_center_id = %s;"""
        ...
        for row in rows :
            params = {
                "job_center": uuid_nuevo,
                "search":  row.get("category_name")
            }
            response = requests.get(api_url +"plague-categories/", headers=headers,params=params)
            if response.status_code == 200:
                data = response.json()
            data = {
                        "name": row.get("plague_name"),
                        "category": data[0]['id'],
                        "job_center": uuid_nuevo
                    }
            resons = requests.post(api_url+"plagues/", headers=headers, json=data)
           


except mysql.connector.Error as e:
    print(f"Error con la tabla plague_types: {e}")
except Exception as ex:
    print(f"Error inesperado con la tabla plague_types: {ex}")
...
try:
    with source_conn.cursor(dictionary=True) as source_cursor:
        query = "SELECT * FROM infestation_degrees WHERE profile_job_center_id = %s"
        source_cursor.execute(query, (id_company_job_center,))
        rows = source_cursor.fetchall()
        for row in rows:
            data = {
            ...
            response = requests.post(api_url+"infestation-degrees/", headers=headers, json=data)
except mysql.connector.Error as e:
    print(f"Error con la tabla infestation_degrees: {e}")
except Exception as ex:
    print(f"Error inesperado con la tabla infestation_degrees: {ex}")
...
try:
    with source_conn.cursor(dictionary=True) as source_cursor:
        query = "SELECT * FROM order_cleanings WHERE profile_job_center_id = %s"
        source_cursor.execute(query, (id_company_job_center,))
        rows = source_cursor.fetchall()
        for row in rows:
            data = {
                ...
                "name_task": "",
                "suggestion": "",
                "aplicate_suggestion": True,
                "job_center": uuid_nuevo,
                "category_cleaning": None
            }
            response = requests.post(api_url+"cleanings/", headers=headers, json=data)
         
except mysql.connector.Error as e:
    print(f"Error con la tabla order_cleanings: {e}")
except Exception as ex:
    print(f"Error inesperado con la tabla order_cleanings: {ex}")
...
try:
    with source_conn.cursor(dictionary=True) as source_cursor:
        ...
        rows = source_cursor.fetchall()
        for row in rows:
            query = """SELECT  p.name FROM  plagues_list pl LEFT JOIN 
		    plague_types p ON pl.plague_id = p.id where pl.price_list_id =  %s ;"""
            source_cursor.execute(query, (row.get("id"),))
            plagues = source_cursor.fetchall()
            pl = []
            for plague in plagues : 
                params = {
                "job_center": uuid_nuevo,
                "search":  plague.get("name")
                }
                ui_plague = requests.get(api_url +"plagues/", headers=headers,params=params)
                if len(ui_plague.json())>0:
                    pl.append( ui_plague.json()[0]['id'])
            params = {
                "job_center": uuid_nuevo,
...
try:
    with source_conn.cursor(dictionary=True) as source_cursor:
        query = "SELECT * FROM price_lists where profile_job_center_id = %s "
        source_cursor.execute(query, (id_company_job_center,))
        rows = source_cursor.fetchall()
        for row in rows:
            data = {
                   "name": row.get("name"),
                    ...
                    "service_type_field": False,
                    "plagues_field": False,
                    "cost_field": False,
                    "ticket_field": False,
                    "group_field": False,
                    "comments_field": False,
                    "mip_inspection_form": False,
                    "mip_condition_form": False,
                    "mip_control_form": False,      
                    "mip_payment_form": False,
                    "mip_signature_form": False,
                    "mip_plan_action_form": False,
                    "mip_station_count_form": False,
                    "mip_task_form": False,
                    "notification_action": False,
                    "reminder_action": False,
                    "folio_key_setting": "OS",
                    "folio_init_setting": 1000,
                    "hide_cancel_setting": False,
                    "is_service_order": False,
                    "job_center": uuid_nuevo
            }
            response = requests.post(api_url+"event-types/", headers=headers, json=data)

except mysql.connector.Error as e:
    print(f"Error con la tabla type_services: {e}")
except Exception as ex:
    print(f"Error inesperado con la tabla type_services: {ex}")
